...
namespace ProjetoMVC01.Controllers
{
    ...
    {
        public IActionResult Cadastro()// método que abre a página de cadastro

        {
            return View();
        }

        [HttpPost] //faz com que o método receba o evento SUBMIT da página (recebe o formulário preenchido
        public IActionResult Cadastro(ProdutoCadastroModel model,
            [FromServices] ProdutoRepository produtoRepository) // fazendo isso eu não preciso instaciar com o  var e new, pq eu já instanciei no startup services.AddTransient(map => new ProdutoRepositor (Configuration.GetConnectionString("_BDProjetoMVC01")));
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    produto.Nome = model.Nome;// eu uso no lugar do Console.ReadLine
                    produto.Preco = Convert.ToDecimal(model.Preco);//Convert.To eu uso no lugar do Parse
                    produto.Quantidade = Convert.ToInt32(model.Quantidade);//Convert.To eu uso no lugar do Parse
        ...
        public IActionResult Consulta([FromServices]ProdutoRepository produtoRepository)
        {
            ...
            var model = new ProdutoConsultaModel();//criou um objeto

            try
            {
                //executar a consulta no banco de dados e armazenar o resultado
            ...
            return View(model);// dentro do model tem a lista de produtos
        }
        ...
        public IActionResult Relatorio(ProdutoRelatorioModel model,
            [FromServices] ProdutoRepository produtoRepository)
        {
        ...
        //método que será chamado(executado) por um código JS localizado em alguma página no sistema
        // O método JsonResult vai "desenhar" o gráfico( ObterDadosGrafico vai no banco de dados,executa a procedure ConsultarTotal e devolve esses dados para o JS montar o gráfico. O JS vai ler esses dados através do código JS programado na index.JsonResult é que vai acessar esse código!
        public JsonResult ObterDadosGrafico([FromServices] ProdutoRepository produtoRepository)
        {
            try
            {
                //retornar para o javascript, o conteudo da consulta feita no banco de dados..
