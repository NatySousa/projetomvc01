    ...
    public class ProdutoReportExcel
    {
        public byte[] GerarExcel(DateTime dataMin, DateTime dataMax, List<Produto> produtos)
        {
            ...
            using (var excelPackage = new ExcelPackage())
            {
                ...
                var linha = 7;// por que é a ultima linha que vai ser usada, poderia ser mais se tivessem mais itens
                ...
                foreach (var item in produtos)
                {
                    ...
                    sheet.Cells[$"D{linha}"].Formula = $"=B{linha}*C{linha}";//fórmula para calcular e atualizar os valores
                    sheet.Cells[$"E{linha}"].Value = item.DataCadastro.ToString("dd/MM/yyyy");

                    linha++;//soma mais 1 para continuar o loping da matriz
                }
